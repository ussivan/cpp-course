...
big_integer::big_integer(int a): sign(a >= 0 ? 0 : max_ui), digits(static_cast<ui>(a)) {}
...
big_integer &big_integer::operator+=(ui rhs) {
    ...
    for (size_t i = 0; i < digits.size(); i++) {
        ui digit = i > 0 ? 0 : rhs;
        ull sum = 1ull * data[i] + carry + digit;
        carry = 0;
        if (sum > max_ui) {
            carry = 1;
            sum -= max_ui + 1;
        }
        data[i] = static_cast<ui>(sum);
    }
    sign = digits.back();
    cut();
    return *this;
...
big_integer &big_integer::operator+=(big_integer const &rhs) {
    ui carry = 0;
    ui const* rhs_data = rhs.digits.data();
    digits.resize(std::max(digits.size(), rhs.digits.size()) + 2, sign);
    ui* data = digits.data();
    for (size_t i = 0; i < digits.size(); i++) {
        ui digit = i >= rhs.digits.size() ? rhs.sign : rhs_data[i];
        ull sum = 1ull * data[i] + carry + digit;
        carry = 0;
        if (sum > max_ui) {
            carry = 1;
...
big_integer operator*(big_integer a, ui b) {
    ui carry = 0;
    ui* data = a.digits.data();
    for (size_t i = 0; i < a.digits.size(); i++) {
        ull sum = 1ull * data[i] * b + carry;
        carry = static_cast<ui>(sum / (1ull + max_ui));
        data[i] = static_cast<ui>(sum % (1ull + max_ui));
    }
    if (carry) {
...
std::pair<big_integer, ui> divmod_by_digit(big_integer a, ui digit) {
    ...
    for (size_t i = a.digits.size(); i-- > 0;) {
        ull cur = data[i] + carry * (1ull + max_ui);
        data[i] = static_cast<ui>(cur / digit);
...
big_integer &big_integer::operator>>=(int rhs) {
    ...
    for (size_t i = digits.size(); i-- > 0;) {
        ui old_digit = data[i];
        data[i] = ((carry << (MAX_2 - left)) | (data[i] >> left));
...
bool operator<(big_integer const &a, big_integer const &b) {
    ...
    ui const* a_data = a.digits.data();
    ui const* b_data = b.digits.data();
    for (size_t i = a.digits.size(); i-- > 0;) {
